...
export function VoteForm() {
  ...
  const [picks, setPicks] = useState<Record<TrackId, number | null>>({
    athletic_performance: null,
    accessibility_solutions: null,
    entrepreneurial_ai: null,
  });
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [success, setSuccess] = useState(false);

  const anyPicked = Object.values(picks).some((v) => v != null);

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
    if (!anyPicked) {
      setError("Pick at least one team to vote for.");
      return;
    }
    setSubmitting(true);
    try {
      const body = {
        email: email.trim(),
        picks: {
          athletic_performance: picks.athletic_performance ?? undefined,
          accessibility_solutions: picks.accessibility_solutions ?? undefined,
          entrepreneurial_ai: picks.entrepreneurial_ai ?? undefined,
        },
      };
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="mt-6 space-y-6">
        {TRACK_ORDER.map((trackId) => {
          ...
          return (
            <div key={trackId}>
              <h3 className="text-base font-bold text-gray-900">{track.label}</h3>
              <div className="mt-3 space-y-2">
                {track.teams.map((t) => {
                  const selected = picks[trackId] === t.team_number;
                  return (
                    <button
                      type="button"
                      key={t.team_number}
                      onClick={() =>
                        setPicks((prev) => ({
                          ...prev,
                          [trackId]: selected ? null : t.team_number,
                        }))
                      }
      ...
      <Button
        type="submit"
        disabled={submitting || !email || !anyPicked}
